import db from '../config/db.js';

// --- ID AKUN BARU YANG SUDAH SINKRON ---
const AKUN_KAS_ID = 11;
const JENIS_SIMPANAN_TO_AKUN_ID = {
  'Pokok': 22,
  'Wajib': 23,
  'Sukarela': 21,
  'Lebaran': 24,
  'Qurban': 25,
  'Lain-lain': 26
};

// --- FUNGSI GET ---

// FUNGSI BARU: Mengambil semua rekening dari semua anggota
export const getAllRekening = async (req, res) => {
  try {
    const query = `
      SELECT 
        r.id, r.no_rekening, r.jenis_simpanan, r.saldo,
        a.id as anggota_id, a.nama as nama_anggota 
      FROM 
        rekening_simpanan r
      JOIN 
        anggota a ON r.anggota_id = a.id
      ORDER BY 
        a.nama ASC, r.jenis_simpanan ASC;
    `;
    const result = await db.query(query);
    res.status(200).json(result.rows);
  } catch (error) {
    console.error('Error mengambil semua rekening:', error);
    res.status(500).json({ message: 'Gagal mengambil data rekening' });
  }
};

// Fungsi untuk mengambil detail satu rekening berdasarkan ID
export const getRekeningById = async (req, res) => {
  const { id } = req.params;
  try {
    const query = 'SELECT * FROM rekening_simpanan WHERE id = $1;';
    const result = await db.query(query, [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ message: 'Rekening tidak ditemukan' });
    }
    res.status(200).json(result.rows[0]);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Error mengambil data rekening.' });
  }
};

// Fungsi untuk mengambil semua rekening milik satu anggota
export const getRekeningByAnggotaId = async (req, res) => {
  const { anggotaId } = req.params;
  try {
    const query = 'SELECT * FROM rekening_simpanan WHERE anggota_id = $1 ORDER BY jenis_simpanan ASC;';
    const result = await db.query(query, [anggotaId]);
    res.status(200).json(result.rows);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Error mengambil data rekening.' });
  }
};

// Fungsi untuk mengambil riwayat transaksi (mutasi) satu rekening
export const getMutasiByRekeningId = async (req, res) => {
    const { rekeningId } = req.params;
    try {
      const query = 'SELECT * FROM transaksi_simpanan WHERE rekening_id = $1 ORDER BY tgl_transaksi DESC;';
      const result = await db.query(query, [rekeningId]);
      res.status(200).json(result.rows);
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Error mengambil data mutasi.' });
    }
  };


// --- FUNGSI CREATE (POST) ---

// Fungsi untuk membuat rekening baru
export const createRekening = async (req, res) => {
  const { anggota_id, jenis_simpanan } = req.body;
  const no_rekening = `${jenis_simpanan.substring(0,2).toUpperCase()}-${String(anggota_id).padStart(4,'0')}`;
  try {
    const query = `INSERT INTO rekening_simpanan(anggota_id, jenis_simpanan, no_rekening, saldo) VALUES($1, $2, $3, 0) RETURNING *;`;
    const newRekening = await db.query(query, [anggota_id, jenis_simpanan, no_rekening]);
    res.status(201).json(newRekening.rows[0]);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Error membuat rekening baru.' });
  }
};

// Fungsi untuk melakukan setoran dana
export const createSetoran = async (req, res) => {
  const { rekeningId } = req.params;
  const { jumlah } = req.body;
  
  const client = await db.connect();
  try {
    await client.query('BEGIN');

    const rekInfoQuery = `SELECT r.no_rekening, r.jenis_simpanan, a.nama FROM rekening_simpanan r JOIN anggota a ON r.anggota_id = a.id WHERE r.id = $1`;
    const rekInfo = await client.query(rekInfoQuery, [rekeningId]);
    const { no_rekening, jenis_simpanan, nama } = rekInfo.rows[0];
    
    const akunSimpananId = JENIS_SIMPANAN_TO_AKUN_ID[jenis_simpanan];
    if (!akunSimpananId) throw new Error(`Jenis simpanan "${jenis_simpanan}" tidak memiliki pemetaan akun.`);

    const keterangan = `Setoran ${jenis_simpanan} ${no_rekening} a.n. ${nama}`;

    await client.query(`INSERT INTO transaksi_simpanan(rekening_id, jenis_transaksi, jumlah) VALUES($1, 'Setor', $2);`, [rekeningId, jumlah]);
    await client.query('UPDATE rekening_simpanan SET saldo = saldo + $1 WHERE id = $2;', [jumlah, rekeningId]);
    
    const jurnalQuery = `INSERT INTO jurnal_umum(keterangan, akun_id, debit, kredit) VALUES ($1, $2, $3, $4);`;
    await client.query(jurnalQuery, [keterangan, AKUN_KAS_ID, jumlah, 0]);
    await client.query(jurnalQuery, [keterangan, akunSimpananId, 0, jumlah]);

    await client.query('COMMIT');
    res.status(200).json({ message: 'Setoran berhasil' });

  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Error saat transaksi setoran:', error);
    res.status(500).json({ message: 'Transaksi setoran gagal' });
  } finally {
    client.release();
  }
};

// Fungsi untuk melakukan penarikan dana
export const createPenarikan = async (req, res) => {
  const { rekeningId } = req.params;
  const { jumlah } = req.body;

  if (!jumlah || jumlah <= 0) return res.status(400).json({ message: 'Jumlah penarikan tidak valid' });

  const client = await db.connect();
  try {
    await client.query('BEGIN');

    const rekInfoQuery = `SELECT r.no_rekening, r.jenis_simpanan, a.nama, r.saldo FROM rekening_simpanan r JOIN anggota a ON r.anggota_id = a.id WHERE r.id = $1 FOR UPDATE`;
    const rekInfo = await client.query(rekInfoQuery, [rekeningId]);
    if (rekInfo.rows.length === 0) throw new Error('Rekening tidak ditemukan');

    const { no_rekening, jenis_simpanan, nama, saldo } = rekInfo.rows[0];

    if (parseFloat(saldo) < jumlah) {
      await client.query('ROLLBACK');
      client.release();
      return res.status(400).json({ message: 'Saldo tidak mencukupi' });
    }
    
    const akunSimpananId = JENIS_SIMPANAN_TO_AKUN_ID[jenis_simpanan];
    if (!akunSimpananId) throw new Error(`Jenis simpanan "${jenis_simpanan}" tidak memiliki pemetaan akun.`);
    
    const keterangan = `Penarikan ${jenis_simpanan} ${no_rekening} a.n. ${nama}`;

    await client.query('UPDATE rekening_simpanan SET saldo = saldo - $1 WHERE id = $2;', [jumlah, rekeningId]);
    await client.query(`INSERT INTO transaksi_simpanan(rekening_id, jenis_transaksi, jumlah) VALUES($1, 'Tarik', $2);`, [rekeningId, jumlah]);

    const jurnalQuery = `INSERT INTO jurnal_umum(keterangan, akun_id, debit, kredit) VALUES ($1, $2, $3, $4);`;
    await client.query(jurnalQuery, [keterangan, akunSimpananId, jumlah, 0]);
    await client.query(jurnalQuery, [keterangan, AKUN_KAS_ID, 0, jumlah]);
    
    await client.query('COMMIT');
    res.status(200).json({ message: 'Penarikan berhasil' });

  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Error saat transaksi penarikan:', error);
    res.status(500).json({ message: 'Transaksi penarikan gagal' });
  } finally {
    client.release();
  }
};

// Tambahkan fungsi baru ini di controller Anda
export const getAllTransaksi = async (req, res) => {
  try {
    const query = `
      SELECT 
        t.id, t.tgl_transaksi, t.jenis_transaksi, t.jumlah,
        r.no_rekening,
        a.nama as nama_anggota
      FROM 
        transaksi_simpanan t
      JOIN 
        rekening_simpanan r ON t.rekening_id = r.id
      JOIN 
        anggota a ON r.anggota_id = a.id
      ORDER BY 
        t.tgl_transaksi DESC;
    `;
    const result = await db.query(query);
    res.status(200).json(result.rows);
  } catch (error) {
    console.error('Error mengambil semua transaksi simpanan:', error);
    res.status(500).json({ message: 'Gagal mengambil data transaksi' });
  }
};

